...
class XDecoder_Trainer(DefaultTrainer):
    ...
    def create_optimizer_and_scheduler(self):
        ...
        for _module_name in self.model_names: # 检查name是否在fix_param中,如果是并且value为True,则将requires_grad设为False,即冻结该参数
        ...
        for _module_name in self.model_names: #为每个参数生成优化器的参数配置hparams,并输出为一个列表params
            ...
            def maybe_add_full_model_gradient_clipping(optim):
                ...
                class FullModelGradientClippingOptimizer(optim): # 这个自定义优化器重载了step()方法,使用torch.nn.utils.clip_grad_norm_()来裁剪全模型的参数梯度范数。
