...
class XDecoderPLUS(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # bbox预测分支
        if task_switch['bbox']:
            self.bbox_embed = MLP(hidden_dim, hidden_dim, 4, 3) # TODO：是否需要多加几层，像 openSeed
        ...
        # 包括object query, class query和caption query之间的交互
        self_attn_mask = torch.zeros((1, num_queries + contxt_len, num_queries + contxt_len)).bool()
        # object和class query不与caption query交互
        self_attn_mask[:, :num_queries, num_queries:] = True
        # caption query只与之前的token交互
        self_attn_mask[:, num_queries:, num_queries:] = torch.triu(torch.ones((1, contxt_len, contxt_len)), diagonal=1).bool()
        # object query不与class query交互
        self_attn_mask[:, :num_queries-1, num_queries-1:num_queries] = True
        # class query不与object query交互
    ...
    def forward(self, x, mask_features, mask=None, target_queries=None, target_vlp=None, task='seg', extra={}):
        if task == 'captioning_infer':
            # 如果是inference阶段的caption生成,直接调用forward_captioning
        ...
        if self.training and task == 'vlp' and self.task_switch['captioning']:
            ...
            caping_lang_embed = torch.cat([caption['caption_tokens'] for caption in target_vlp], dim=0).transpose(0, 1) # language output  seq_len x bs x hidden_dim shape
            ...
            # 构建填充后的完整mask
            # mask大小 = 对象查询数 + 对象查询数-1 + grounding词个数
            # 初始化为全1的bool mask
            ...
            #TODO: 可以直接用来做object attention
            self_tgt_mask = self.self_attn_mask[:,:self.num_queries,:self.num_queries].repeat(output.shape[1]*self.num_heads, 1, 1)


        # prediction heads on learnable query features # TODO：为什么先出结果？ query的self attention?
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, layer_id=-1, task='seg'):
        ...
        obj_token = norm_decoder_output[:,:self.num_queries-1] # 提取object token
        cls_token = norm_decoder_output[:,self.num_queries-1:self.num_queries] # 提取class token

        sim = (cls_token @ obj_token.transpose(1,2)).softmax(-1)[:,0,:,None] # # 计算相似度  用来干嘛的？ TODO include class token.
        # 用object token和class token 的相似度加权求和生成新的class token
        ...
        if self.task_switch['mask']:
            ...
            # 转换为0到1之间的概率 基于概率创建注意力遮盖 - 预测概率<0.5的位置为True
            attn_mask = (attn_mask.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0, 1) < 0.5).bool()
            attn_mask = attn_mask.detach()

            # NOTE: fill False for cls token (JY)
            # 将CLS标记位置的注意力遮盖设置为False
...
def get_masked_transformer_decoder(cfg, in_channels, lang_encoder, mask_classification, extra):
    return XDecoderPLUS(cfg, in_channels, lang_encoder, mask_classification, extra)